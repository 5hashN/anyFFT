...
def get_cupy_types(dtype_str):
    """Returns (real_dtype, complex_dtype) for CuPy."""
...
def test_r2c_out_of_place(dtype_str, shape, ndim):
    ...
    real_dtype, complex_dtype = get_cupy_types(dtype_str)
    ...
    # Alloc on GPU
    # Note: cufft backend constructor in C++ checks shape but doesn't strictly require
    # the dummy arrays for planning like FFTW often does, but we pass them to match API.
...
def test_r2c_in_place(dtype_str, shape, ndim):
    ...
    # Fill Valid Data
    valid_n = shape[-1]
    clean_input = cp.random.rand(*shape).astype(real_dtype)
...
def test_c2c_out_of_place(real_dtype_str, shape, ndim):
    ...
    _, complex_dtype = get_cupy_types(real_dtype_str)

    # Data
    in_data = cp.random.rand(*shape).astype(complex_dtype) + \
              1j * cp.random.rand(*shape).astype(complex_dtype)
...
def test_c2c_in_place(real_dtype_str, shape, ndim):
    ...
    # Data
    clean_input = cp.random.rand(*shape).astype(complex_dtype) + \
                  1j * cp.random.rand(*shape).astype(complex_dtype)
